...
A retained local-only delivery, or a retained merge link from a forge other than GitHub, cannot reach the row because both travel as `--note`, which exists on `tasks-axi done` but not on `tasks-axi update`, while the durable pending-close record carrying that note is retired when retention completes.
...
When an interrupted retention leaves such a relocated report, or a non-GitHub merge link that an older teardown recorded under `--pr`, in the validated pending-close record, `answer` skips only that known-unsupported row artifact and closes normally, so the delivery remains absent from Recently Landed instead of wedging the captain's answer.
